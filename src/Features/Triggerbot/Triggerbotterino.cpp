...
{
	if (!gCheatMenu.triggerbot_active)
		return;
    ...
    Vector eyePosition = pLocal->GetEyePosition();
	Ray_t ray;
	trace_t trace;
	CTraceFilter filt;
	...
	if (gCheatMenu.triggerbot_headonly && trace.hitbox != 0)
		return;
